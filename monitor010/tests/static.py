...
R=Path(__file__).resolve().parents[2];base=(R/'out/madre2-baseline.html').read_text();new=(R/'out/LAB_011_FAILOVER.html').read_text()
assert hashlib.sha256(base.encode()).hexdigest()=='42aead77c8cac5c36d3eb9ec7cf81b9efb69f8fd5979ebd52a5e4ca01c28cb9b'
# Compare every byte outside the intentionally changed key/diagnostics subsystem and injected assets.
def normalize(s):
 s=re.sub(r'<script id="gv-key-state-011">[\s\S]*?</script>\s*','',s)
 s=re.sub(r'<style id="gv-monitor-style">[\s\S]*?</style>','',s)
 s=re.sub(r'<script id="gv-places-failover-011">[\s\S]*?</script>\s*','',s)
 s=re.sub(r'<script id="gv-monitor-011">[\s\S]*?</script>\s*','',s)
 s=re.sub(r'(?:let gvRetry011=0;\n)?function gvDiagAdvanceGoogleKey\(e\)[\s\S]*?(?=\ngvDiagApplySelectedKey\(\);)','KEY_FAILOVER',s)
...
# Normal Places failover must not reload the page. Reload is allowed only in explicit gm_authFailure edge handling.
...
for folder,app_id in [('android-keybox','it.geovision.keybox'),('android-youtube-test','it.geovision.lab.failover011')]:
...
print('PASS: Mother hash; isolated modifications; JS syntax; in-place Places failover; manual switches; secure sync; package/version IDs; vault compatibility')
