...
(async()=>{
 ...
 const server=http.createServer((req,res)=>{res.setHeader('Content-Type','text/html');res.end(fs.readFileSync(root+'/out/'+(req.url.includes('baseline')?'madre2-baseline.html':'LAB_012_FAILOVER.html')));});
 ...
 // The manual verify button must NOT probe Google: Google status comes from real GeoVision requests only.
 await page.getByRole('button',{name:'Verifica AI / YouTube',exact:true}).click();await page.waitForFunction(()=>document.getElementById('gvMonitorStatus').textContent.startsWith('AI e YouTube verificati'));assert.equal(apiRequests,2);assert.equal(await page.locator('[data-name="ai"] .dot[data-state="ok"]').count(),1);assert.equal(await page.locator('[data-name="youtube"] .dot[data-state="bad"]').count(),1);assert.match(await page.locator('[data-name="google1"] .state').innerText(),/attende uso reale/i);
 ...
 // Restore G1 and force it active. A real Places 429 must mark G1 red, switch immediately to G2, retry in place and mark G2 green.
 ...
 assert.match(await page.locator('[data-name="google1"] .state').innerText(),/Quota Google esaurita/);assert.equal(await page.locator('[data-name="google1"] .dot').getAttribute('data-state'),'bad');assert.match(await page.locator('[data-name="google2"] .state').innerText(),/ATTIVA · Places operativo/);assert.equal(await page.locator('[data-name="google2"] .dot').getAttribute('data-state'),'ok');
 ...
 console.log('PASS browser LAB 012: no synthetic Google probes; real-request status; 3 manual switches; immediate 429 G1->G2 retry; disabled key skipped; all-OFF stop; no Places reload; legacy contradictory diagnostics absent; Mother card/social markup preserved.');
