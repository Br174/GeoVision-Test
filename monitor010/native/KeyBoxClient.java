...
public final class KeyBoxClient {
    ...
    private void accept(Intent data){try{JSONObject p=new JSONObject();for(String n:new String[]{"google1","google2","google3","ai","youtube"}){String v=data.getStringExtra(n);if(v==null||v.length()>512)throw new IllegalArgumentException();p.put(n,v);}p.put("revision",data.getLongExtra("revision",0));pending=p.toString();deliver();}catch(Exception e){error("Risposta KeyBox incompleta: chiavi locali conservate");}}
