/* Transparent Places REST failover. Google status comes only from real GeoVision requests. */
(function(root){
'use strict';
...
const RUNTIME='geovision_google_runtime_status_v2';
...
function runtimeText(e,status){const t=errText(e);if(status===429||/RESOURCE_EXHAUSTED|OVER_QUERY_LIMIT|quota/i.test(t))return 'Quota Google esaurita';if(/REQUEST_DENIED|RefererNotAllowed|ApiNotActivated|BillingNotEnabled|invalid|not valid|denied/i.test(t))return 'Accesso Google negato';return status?'Errore Google · HTTP '+status:'Errore Google';}
function mark(index,ok,text){if(index<0||index>2)return;let all={};try{all=JSON.parse(localStorage.getItem(RUNTIME)||'{}')||{};}catch{}all[String(index)]={ok:!!ok,text:String(text||''),ts:Date.now()};try{localStorage.setItem(RUNTIME,JSON.stringify(all));root.dispatchEvent(new CustomEvent('gv-google-runtime',{detail:{index,ok:!!ok,text}}));}catch{}}
...
async function request(path,opt={}){
 ...
 for(let attempt=0;attempt<3;attempt++){
  ...
  let r;
  try{r=await fetch('https://places.googleapis.com/v1/'+path,{...opt,headers});}catch(e){mark(a.idx,false,'Errore rete Google');throw e;}
  if(r.ok){mark(a.idx,true,'Places operativo');return r.status===204?{}:r.json();}
  const text=await r.text();last=bodyError(r.status,text);mark(a.idx,false,runtimeText(last,r.status));
  if(!s.keyFailure(last)||!s.advance(last))throw last;
 }
...
function install(){
 if(!root.google?.maps?.importLibrary||root.google.maps.importLibrary.__gvFailover)return false;
 const original=root.google.maps.importLibrary.bind(root.google.maps);root.gvMapsBootKey=localStorage.getItem('geovision_google_maps_api_key')||'';
 const wrapped=async function(name){const lib=await original(name);if(name!=='places'||!lib?.Place||lib.Place.__gvFailover)return lib;const P=lib.Place,origSearch=P.searchByText?.bind(P),origNearby=P.searchNearby?.bind(P),origFetch=P.prototype?.fetchFields;
  P.searchByText=async function(o){const a=state().active();if(a.key&&a.key===root.gvMapsBootKey&&a.enabled[a.idx]&&origSearch){try{const v=await origSearch(o);mark(a.idx,true,'Places operativo');return v;}catch(e){mark(a.idx,false,runtimeText(e,e?.status));if(!state().keyFailure(e))throw e;if(!state().advance(e))throw e;}}return searchText(o);};
  P.searchNearby=async function(o){const a=state().active();if(a.key&&a.key===root.gvMapsBootKey&&a.enabled[a.idx]&&origNearby){try{const v=await origNearby(o);mark(a.idx,true,'Places operativo');return v;}catch(e){mark(a.idx,false,runtimeText(e,e?.status));if(!state().keyFailure(e))throw e;if(!state().advance(e))throw e;}}return searchNearby(o);};
  if(origFetch)P.prototype.fetchFields=async function(o){const a=state().active();if(a.key&&a.key===root.gvMapsBootKey&&a.enabled[a.idx]){try{const v=await origFetch.call(this,o);mark(a.idx,true,'Place Details operativo');return v;}catch(e){mark(a.idx,false,runtimeText(e,e?.status));if(!state().keyFailure(e))throw e;if(!state().advance(e))throw e;}}await details(this);return {place:this};};
  P.__gvFailover=true;return lib;
 };
 wrapped.__gvFailover=true;root.google.maps.importLibrary=wrapped;return true;
}
root.GVPlacesFailover={install,request,searchText,searchNearby,details,compat,mark};
})(window);
