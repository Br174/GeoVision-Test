...
for i in (1, 2, 3):
    old = (f'<div class="key-input-wrap"><input id="mapsKey{i}" type="password" '
    ...
    new = (f'<div class="key-input-wrap"><input id="mapsKey{i}" type="password" '
           f'placeholder="Chiave Google Maps {i}"><button class="key-eye" type="button" '
    ...
    if old not in s: raise SystemExit(f"UI chiave {i} non trovata")
    s = s.replace(old, new, 1)

anchor_total = '<small id="mapsKeyActive">Chiave attiva: —</small>'
if anchor_total not in s: raise SystemExit("Anchor totale switch non trovato")
s = s.replace(anchor_total, anchor_total + '\n  <small id="mapsSwitchTotal">Totale oggi: 0</small>', 1)
    ...
    try{
        ...
        if(raw){
            ...
            if(parsed && parsed.day===today && Array.isArray(parsed.counts)){
                stats={day:today,counts:[0,1,2].map(i=>Math.max(0,Number(parsed.counts[i])||0)),total:Math.max(0,Number(parsed.total)||0)};
            }
        }
...
if anchor not in s: raise SystemExit("Anchor funzioni chiavi non trovato")
...
if old_switch not in s: raise SystemExit("Funzione switchGoogleMapKey non trovata")
...
if fill_anchor not in s: raise SystemExit("Anchor fillKeysPanel non trovato")
...
for token in ('mapsSwitchCount1','mapsSwitchCount2','mapsSwitchCount3','mapsSwitchTotal','gvRecordGoogleAutoSwitch(previous)'):
    if token not in s: raise SystemExit(f"Controllo fallito: {token}")
