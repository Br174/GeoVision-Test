...
CAND_HTML_SHA='42aead77c8cac5c36d3eb9ec7cf81b9efb69f8fd5979ebd52a5e4ca01c28cb9b'
...
# CANDIDATA PULITA: riparte dalla MADRE ORIGINALE e riceve solo il metodo FOTO finale.
...
test "$(sha256sum /tmp/GeoVision_LAB_001_SOURCE.html | awk '{print $1}')" = "$CAND_HTML_SHA"
...
! grep -q 'gvLocalityPrecision004' /tmp/GeoVision_LAB_001_SOURCE.html
...
s=re.sub(r"applicationId '[^']+'", "applicationId 'it.geovision.cand.photo001'", s, count=1)
s=re.sub(r'versionCode\s+\d+', 'versionCode 1801', s, count=1)
s=re.sub(r"versionName '[^']+'", "versionName '1.0-cand-photo-001'", s, count=1)
...
x=re.sub(r'android:label="[^"]+"', 'android:label="CAND 001 FOTO"', x, count=1)
...
"$APKSIGNER" verify --print-certs "$APK" | tee /tmp/cand001-signer.txt
ACTUAL=$(grep -i 'certificate SHA-256 digest:' /tmp/cand001-signer.txt | head -1 | awk '{print $NF}' | tr '[:upper:]' '[:lower:]' | tr -d ':')
test "$ACTUAL" = "$EXPECTED_SIGNER"
"$AAPT" dump badging "$APK" | tee /tmp/cand001-badging.txt
grep -q "package: name='it.geovision.cand.photo001'" /tmp/cand001-badging.txt
grep -q "application-label:'CAND 001 FOTO'" /tmp/cand001-badging.txt
unzip -p "$APK" assets/geovision.html > /tmp/packaged.html
test "$(sha256sum /tmp/packaged.html | awk '{print $1}')" = "$CAND_HTML_SHA"
...
cp "$APK" CAND_001_FOTO.apk
cp /tmp/GeoVision_LAB_001_SOURCE.html CAND_001_FOTO.html
APK_SHA=$(sha256sum CAND_001_FOTO.apk | awk '{print $1}')
cat > MANIFEST_CAND_001_FOTO.txt <<EOF
CAND 001 FOTO
Base: MADRE ORIGINALE pulita (madre/001-social-keybox-stabile).
Metodo importato: esclusivamente la correzione FOTO approvata in LAB 003 FOTO.
La LAB non e' stata convertita in candidata: il metodo finale e' stato riapplicato alla MADRE ORIGINALE pulita.
Escluso esplicitamente il metodo FRAZIONI/LAB 004.
Package isolato: it.geovision.cand.photo001
VersionCode: 1801
VersionName: 1.0-cand-photo-001
Firma SHA256: $EXPECTED_SIGNER
HTML MADRE ORIGINALE SHA256: $MADRE_HTML_SHA
HTML CANDIDATA SHA256: $CAND_HTML_SHA
APK SHA256: $APK_SHA
Se approvata dopo test approfondito, questo stesso sorgente pulito sara' promosso a MADRE ORIGINALE 1.
EOF
zip -j CAND_001_FOTO.zip CAND_001_FOTO.apk CAND_001_FOTO.html MANIFEST_CAND_001_FOTO.txt
sha256sum CAND_001_FOTO.apk CAND_001_FOTO.html CAND_001_FOTO.zip
