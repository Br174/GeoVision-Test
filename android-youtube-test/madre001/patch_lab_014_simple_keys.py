...
# Disable automatic Google key switching. Manual panel chooses the first enabled Google key only.
s=s.replace('function gvDiagAdvanceGoogleKey(e) {','function gvDiagAdvanceGoogleKey(e) { return false; /* LAB014 no auto switch */\n/*',1)
anchor='\ngvDiagApplySelectedKey();'
pos=s.find(anchor)
if pos>0:
    # close comment before first boot apply call
    s=s[:pos]+'*/\n'+s[pos:]
js=r'''
...
async function verifyAll(){if(!panel)return;for(const [n,l,k] of ITEMS){const dot=panel.querySelector('[data-n="'+n+'"] .dot'),sub=panel.querySelector('[data-n="'+n+'"] .sub');if(!dot)continue;dot.className='dot wait';const ok=await verify(n,saved(n,k));if(!panel)return;dot.className='dot '+(ok?'ok':'bad');if(!n.startsWith('google'))sub.textContent=(enabled(n)?'ON':'OFF')+' · '+(ok?'funziona':'non disponibile');else if(!ok)sub.textContent=(enabled(n)?'ON':'OFF')+' · non disponibile';}updateActive();}
