...
cat \
  ...
  | tr -d '\n\r ' | base64 -d | gzip -d > /tmp/GeoVision_LAB_BASE_MADRE001.html

test "$(sha256sum /tmp/GeoVision_LAB_BASE_MADRE001.html | awk '{print $1}')" = "$SOURCE_SHA"
cp /tmp/GeoVision_LAB_BASE_MADRE001.html /tmp/GeoVision_MADRE_001_SOURCE.html
...
s=re.sub(r"versionName '[^']+'", "versionName '1.0-lab-base-madre001-001'", s, count=1)
...
x=re.sub(r'android:label="[^"]+"', 'android:label="GeoVision LAB BASE 001"', x, count=1)
...
"$APKSIGNER" verify --print-certs "$APK" | tee /tmp/lab-base001-signer.txt
ACTUAL=$(grep -i 'certificate SHA-256 digest:' /tmp/lab-base001-signer.txt | head -1 | awk '{print $NF}' | tr '[:upper:]' '[:lower:]' | tr -d ':')
test "$ACTUAL" = "$EXPECTED_SIGNER"
"$AAPT" dump badging "$APK" | tee /tmp/lab-base001-badging.txt
grep -q "package: name='it.geovision.lab.madre001base001'" /tmp/lab-base001-badging.txt
grep -q "application-label:'GeoVision LAB BASE 001'" /tmp/lab-base001-badging.txt
...
cp "$APK" GeoVision_LAB_BASE_MADRE001_001.apk
cp /tmp/GeoVision_MADRE_001_SOURCE.html GeoVision_LAB_BASE_MADRE001_001.html
APK_SHA=$(sha256sum GeoVision_LAB_BASE_MADRE001_001.apk | awk '{print $1}')
cat > MANIFEST_LAB_BASE_MADRE001_001.txt <<EOF
GeoVision LAB BASE MADRE001 001
Copia laboratorio della MADRE 001 approvata.
Nessuna modifica funzionale rispetto alla MADRE 001.
Scopo: punto di partenza isolato per i prossimi esperimenti.
Funzioni presenti: scheda Google ufficiale, foto ufficiali Google, KeyBox, failover chiavi Google, TTS, Instagram nativo, TikTok nativo, UI/pulsanti approvati.
Package isolato: it.geovision.lab.madre001base001
VersionCode: 1601
VersionName: 1.0-lab-base-madre001-001
Firma SHA256: $EXPECTED_SIGNER
HTML MADRE APPROVATO SHA256: $APPROVED_HTML_SHA
APK SHA256: $APK_SHA
Regola: questo LAB può essere modificato; la MADRE 001 resta congelata.
EOF
zip -j GeoVision_LAB_BASE_MADRE001_001.zip \
  GeoVision_LAB_BASE_MADRE001_001.apk \
  GeoVision_LAB_BASE_MADRE001_001.html \
  MANIFEST_LAB_BASE_MADRE001_001.txt
sha256sum GeoVision_LAB_BASE_MADRE001_001.apk GeoVision_LAB_BASE_MADRE001_001.html GeoVision_LAB_BASE_MADRE001_001.zip
