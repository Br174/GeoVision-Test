...
APPROVED_MOTHER_HTML_SHA='a9968ed9870cdeeb27613b73f4e2ef5446696d3810e302f0282018583c338807'
...
BASE_SHA=$(sha256sum /tmp/GeoVision_MADRE_001_SOURCE.html | awk '{print $1}')
test "$BASE_SHA" = "$APPROVED_MOTHER_HTML_SHA"

# One isolated experiment only: locality/municipality Place ID resolver.
python "$ROOT/madre001/patch_locality_placeid_001.py"
PATCHED_SHA=$(sha256sum /tmp/GeoVision_MADRE_001_SOURCE.html | awk '{print $1}')

grep -q 'LAB LOCALITY PLACEID 001' /tmp/GeoVision_MADRE_001_SOURCE.html
...
s=re.sub(r"applicationId '[^']+'", "applicationId 'it.geovision.lab.locality001'", s, count=1)
s=re.sub(r'versionCode\s+\d+', 'versionCode 1601', s, count=1)
s=re.sub(r"versionName '[^']+'", "versionName '1.0-lab-locality-placeid-001'", s, count=1)
...
x=re.sub(r'android:label="[^"]+"', 'android:label="GeoVision LAB LOCALITY 001"', x, count=1)
...
"$APKSIGNER" verify --print-certs "$APK" | tee /tmp/locality001-signer.txt
ACTUAL=$(grep -i 'certificate SHA-256 digest:' /tmp/locality001-signer.txt | head -1 | awk '{print $NF}' | tr '[:upper:]' '[:lower:]' | tr -d ':')
test "$ACTUAL" = "$EXPECTED_SIGNER"
"$AAPT" dump badging "$APK" | tee /tmp/locality001-badging.txt
grep -q "package: name='it.geovision.lab.locality001'" /tmp/locality001-badging.txt
grep -q "application-label:'GeoVision LAB LOCALITY 001'" /tmp/locality001-badging.txt
unzip -p "$APK" assets/geovision.html > /tmp/packaged.html
test "$(sha256sum /tmp/packaged.html | awk '{print $1}')" = "$PATCHED_SHA"
grep -q 'LAB LOCALITY PLACEID 001' /tmp/packaged.html
...
cp "$APK" GeoVision_LAB_LOCALITY_PLACEID_001.apk
cp /tmp/GeoVision_MADRE_001_SOURCE.html GeoVision_LAB_LOCALITY_PLACEID_001.html
APK_SHA=$(sha256sum GeoVision_LAB_LOCALITY_PLACEID_001.apk | awk '{print $1}')
cat > MANIFEST_LAB_LOCALITY_PLACEID_001.txt <<EOF
GeoVision LAB LOCALITY PLACEID 001
Base: MADRE 001 SOCIAL KEYBOX STABILE, device-approved.
Unica modifica funzionale: risoluzione città/comuni via Google Places senza filtro rigido locality.
Obiettivo test: casi come Montoro, dove il nome comunale corretto veniva associato a una frazione/Place ID non ideale.
Priorità: nome esatto + entità geografica/amministrativa + parent coerente + prossimità + foto disponibili.
Scheda Google originale: non modificata.
KeyBox, failover Google, TTS, Instagram, TikTok e UI: invariati.
Package isolato: it.geovision.lab.locality001
Firma SHA256: $EXPECTED_SIGNER
Base Mother HTML SHA256: $BASE_SHA
LAB HTML SHA256: $PATCHED_SHA
APK SHA256: $APK_SHA
EOF
zip -j GeoVision_LAB_LOCALITY_PLACEID_001.zip \
  GeoVision_LAB_LOCALITY_PLACEID_001.apk \
  GeoVision_LAB_LOCALITY_PLACEID_001.html \
  MANIFEST_LAB_LOCALITY_PLACEID_001.txt
sha256sum GeoVision_LAB_LOCALITY_PLACEID_001.apk GeoVision_LAB_LOCALITY_PLACEID_001.html GeoVision_LAB_LOCALITY_PLACEID_001.zip
