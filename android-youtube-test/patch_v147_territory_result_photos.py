...
# v147: nella lista "Foto Google del territorio", il tap su un risultato
...
async function gvV147OpenTerritorySelectedPlace(place){
    ...
    try{
        ...
        console.log('GeoVision v147 selected place fetchFields failed',id,e?.message||e);
    }
    ...
    if(!urls.length && typeof legacyGooglePhotos==='function'){
        ...
        }catch(e){ console.log('GeoVision v147 selected place legacy photos failed',id,e?.message||e); }
    }

    if(urls.length){
        gvV144RenderPhotoFolder({name,placeId:id},urls);
        console.log('GeoVision v147 selected territory place photos active',id,urls.length);
        return;
    }
...
if 'function gvV147OpenTerritorySelectedPlace(place)' not in s:
    if anchor not in s:
        raise SystemExit('v147 patch aborted: googlePlaceUrl anchor not found')
...
if old not in s:
    raise SystemExit('v147 patch aborted: selectable territory search anchor not found')
s=s.replace(old,new,1)

p.write_text(s,encoding='utf-8')
print('Applied v147 territory result -> selected place photos:',p,len(s))
