    ...
    const rows=items.map((item,i)=>{
        ...
        return \`<button type="button" data-yt-index="\${i}" style="border:0;border-bottom:1px solid #eef1f4;background:#fff;padding:10px 12px;display:grid;grid-template-columns:142px 1fr;gap:11px;text-align:left;align-items:start">
          ...
          <div style="min-width:0">
            <b style="display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;font-size:13px;line-height:1.35;color:#344054">\${esc(sn.title||'Video YouTube')}</b>
            <small style="display:block;margin-top:6px;color:#8a94a3;font-size:11px">\${esc(sn.channelTitle||'YouTube')}</small>
          </div>
        </button>\`;
    }).join('');
    modal.innerHTML=\`
      <div style="height:58px;flex:0 0 58px;display:flex;align-items:center;gap:10px;padding:8px 12px;border-bottom:1px solid #e7eaf0;background:#fff">
        <div style="flex:1;min-width:0"><b style="display:block;color:#344054;font-size:14px">Video trovati</b><small style="display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#8a94a3">\${esc(q)}</small></div>
        <button id="youtubeResultsClose" type="button" aria-label="Chiudi" style="width:38px;height:38px;border:1px solid #e5e7eb;border-radius:50%;background:#fff;color:#64748b;font-size:24px">×</button>
      </div>
      <div style="flex:1;overflow:auto;-webkit-overflow-scrolling:touch">\${rows}</div>\`;
    document.body.appendChild(modal);
    document.getElementById('youtubeResultsClose').onclick=closeYouTubeResults;
    modal.querySelectorAll('[data-yt-index]').forEach(btn=>{
        btn.onclick=()=>{
            const item=items[Number(btn.dataset.ytIndex)];
            const id=item?.id?.videoId;
            if(!id)return;
            openYouTubeInternalPlayer(id,item?.snippet?.title||'YouTube');
        };
    });
}
...
window.gvNativeTtsState=function(state){
    ...
    $('#nativeVoice')?.classList.toggle('active',active);
};
speak=function(text,append=false){
    ...
    try{
        if(window.GeoVisionTTS && typeof window.GeoVisionTTS.speak==='function'){
            ...
            window.GeoVisionTTS.speak(t,!!append);
            return;
        }
    }catch{}
