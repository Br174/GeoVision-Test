...
marker = 'GV_PHOTO_URI_LAB_V2'
if marker in s:
    print('Photo URI LAB V2 already present')
    raise SystemExit(0)

# Espone al laboratorio il luogo realmente aperto dalla normale openPlace().
old = "async function openPlace(p) { setTopbarForSheet(true); macroPrimary = ''; current = p; closeNativeAudio();"
new = "async function openPlace(p) { setTopbarForSheet(true); macroPrimary = ''; current = p; window.gvPhotoLabCurrent = p; closeNativeAudio();"
if old not in s:
    raise SystemExit('openPlace anchor not found')
...
<div id="gvPhotoUriLabOverlay" role="dialog" aria-modal="true">
  <div id="gvPhotoUriLabCard">
    <div id="gvPhotoUriLabHead"><div id="gvPhotoUriLabTitle">Laboratorio foto Google V2</div><button id="gvPhotoUriLabClose" type="button">×</button></div>
...
(function(){
  ...
  btn.onclick=async()=>{
    ...
    const p=window.gvPhotoLabCurrent||null;
    if(!p){
      st.textContent='NESSUN LUOGO REGISTRATO\nApri una scheda tramite GeoVision e poi premi TEST FOTO GOOGLE.';
      return;
    }
    if(!p.placeId){
      st.textContent='PLACE ID ASSENTE\nLuogo registrato: '+text(p.name)+'\nLa scheda è stata riconosciuta, ma non contiene un Place ID Google.';
      return;
    }
    ...
    try{
      ...
      if(!photos.length){
        st.textContent='RISULTATO: ZERO FOTO\nLuogo: '+text(p.name)+'\nPlace ID: '+text(p.placeId)+'\nfetchFields completato senza errore, ma photos[] è vuoto.';
        return;
      }
      const ph=photos[0];
      ...
      st.textContent='SUCCESSO: FOTO ESTRATTE DA GOOGLE\nLuogo: '+text(p.name)+'\nPlace ID: '+text(p.placeId)+'\nFoto restituite: '+photos.length+'\nURI ottenuto. Verifico il caricamento nel contenitore GeoVision…';
      const attrs=Array.isArray(ph.authorAttributions)?ph.authorAttributions:[];
      at.textContent=attrs.length?'Attribuzione: '+attrs.map(a=>a.displayName||a.uri||'Google').join(' · '):'Attribuzione foto: nessun testo restituito.';
      im.onload=()=>{
        im.style.display='block';
        st.textContent='SUCCESSO COMPLETO\nLuogo: '+text(p.name)+'\nPlace ID: '+text(p.placeId)+'\nFoto restituite: '+photos.length+'\nPhoto.getURI(): OK\nCaricamento IMG nel contenitore GeoVision: OK';
      };
      im.onerror=()=>{
        im.style.display='none';
        st.textContent='SUCCESSO PARZIALE\nLuogo: '+text(p.name)+'\nFoto restituite: '+photos.length+'\nPhoto.getURI(): OK\nCaricamento IMG: FALLITO';
      };
      im.src=uri;
    }catch(err){
